...
const activateUser = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(
            ...
            {
                select: '-createdBy -password -loginAttempts -confirmationCode',
                new: true
            }
        );
	  ...
	  User.findOne({ 'email': req.body.email }, function (err, user2) {
		  ...
		  if (!user2) {
			  user.save((err, userSaved) => {
				  ...
				  serviceEmail.sendMailVerifyEmail(req.body.email, req.body.userName, req.body.position, req.body.institution)
					  .then(response => {
						 // res.status(200).send({ message: 'Account created' })
					  })
					  .catch(response => {
